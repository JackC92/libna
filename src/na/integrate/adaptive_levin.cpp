...
namespace na
{
	...
	namespace integrate
	{
			...
			const Eigen::Index k)
		{
			...
			std::queue<na::internal::integrate::IntegralTriplet<double>> q;
			q.push({ a, b, na::internal::integrate::adaptive_levin_estimate(f, g, a, b, k, xscheb, D) });
			double val = 0.0;
			while (!q.empty())
			{
				double mid = 0.5 * (q.front().a + q.front().b);
				double val0 = q.front().val;
				double valL = na::internal::integrate::adaptive_levin_estimate(f, g, q.front().a, mid, k, xscheb, D);
				double valR = na::internal::integrate::adaptive_levin_estimate(f, g, mid, q.front().b, k, xscheb, D);
			...
			const Eigen::Index k)
		{
			...
			std::queue<na::internal::integrate::IntegralTriplet<std::complex<double>>> q;
			q.push({ a, b, na::internal::integrate::cadaptive_levin_estimate(f, g, a, b, k, xscheb, D) });
			std::complex<double> val(0.0, 0.0);
			while (!q.empty())
			{
				double mid = 0.5 * (q.front().a + q.front().b);
				std::complex<double> val0 = q.front().val;
				std::complex<double> valL = na::internal::integrate::cadaptive_levin_estimate(f, g, q.front().a, mid, k, xscheb, D);
				std::complex<double> valR = na::internal::integrate::cadaptive_levin_estimate(f, g, mid, q.front().b, k, xscheb, D);
				if (std::abs(val0 - valL - valR) < tol)
				{
					val += val0;
				}
