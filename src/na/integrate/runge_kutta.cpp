#include "na/integrate/runge_kutta.h"
#include <algorithm>
#include <cmath>
#include <functional>

namespace na
{
	bool dopri5(
		const std::function<double(double, double)> f,
		const double t0,
		const double t1,
		const double y0,
		const double tolrel,
		const double tolabs,
		double& y)
	{
		if (t1 == t0)
		{
			y = y0;
			return true;
		}
		
		constexpr double c2 = 0.2, c3 = 0.3, c4 = 0.8, c5 = 8.0 / 9.0;
		constexpr double a21 = 1.0;
		constexpr double a31 = 0.25, a32 = 0.75;
		constexpr double a41 = 11.0 / 9.0, a42 = -14.0 / 3.0, a43 = 40.0 / 9.0;
		constexpr double a51 = 4843.0 / 1458.0, a52 = -3170.0 / 243.0, a53 = 8056.0 / 729.0, a54 = -53.0 / 162.0;
		constexpr double a61 = 9017.0 / 3168.0, a62 = -355.0 / 33.0, a63 = 46732.0 / 5247.0, a64 = 49.0 / 176.0, a65 = -5103.0 / 18656.0;
		constexpr double a71 = 35.0 / 384.0, a73 = 500.0 / 1113.0, a74 = 125.0 / 192.0, a75 = -2187.0 / 6784.0, a76 = 11.0 / 84.0;
		constexpr double e1 = 71.0 / 57600.0, e3 = -71.0 / 16695.0, e4 = 71.0 / 1920.0, e5 = -17253.0 / 339200.0, e6 = 22.0 / 525.0, e7 = -1.0 / 40.0;

		// Maximal number of allowed steps
		constexpr int maxstep = 100000;
		// Step number after which stiffness detection is activated
		constexpr int nstiff = 1000;
		// Machine epsilon for double precision
		constexpr double eps = 2.3e-16;
		constexpr double safe = 0.9;
		constexpr double fac1 = 0.2;
		constexpr double fac2 = 10.0;
		constexpr double beta = 0.04;
		constexpr double expo1 = 0.2 - beta * 0.75;
		constexpr double facc1 = 1.0 / fac1;
		constexpr double facc2 = 1.0 / fac2;
		const double hmax = t1 - t0;

		// Stiffness detection variables
		int iasti = 0;
		int nonsti = 0;
		double hlamb = 0.0;

		bool reject = false;
		double facold = 1e-4;

		y = y0;
		double h;
		double t = t0;
		// Compute a first guess for explicit Euler as
		//     h = 0.01 * norm(y0) / norm(f(t0, y0))
		double f0 = f(t0, y0);
		double sk = tolabs + tolrel * std::abs(y0);
		double dnf = std::pow(f0 / sk, 2.0);
		double dny = std::pow(y0 / sk, 2.0);
		if ((dnf < 1e-10) || (dny < 1e-10))
		{
			h = 1e-6;
		}
		else
		{
			h = 0.01 * std::sqrt(dny / dnf);
		}
		// TODO: allow user-specified maximal step size
		h = std::min<double>(h, t1 - t0);
		double f1 = f(t0 + h, y0 + h * f0);
		double der2 = std::abs((f1 - f0) / sk) / h;
		// Step size is computed such that
		//     h**5 * max(norm(f(t0, y0)), norm(der2)) = 0.01
		double der12 = std::max<double>(der2, std::sqrt(dnf));
		if (der12 < 1e-15)
		{
			h = std::min<double>({ 100.0 * h, std::max<double>(1e-6, h * 1e-3), t1 - t0 });
		}
		else
		{
			h = std::min<double>({ 100.0 * h, std::pow(0.01 / der12, 0.2), t1 - t0 });
		}

		int step = 0;
		int naccept = 0;
		int nreject = 0;
		while (true)
		{
			if (step > maxstep)
			{
				return false;
			}
			if (0.01 * h < std::abs(t) * eps)
			{
				return false;
			}
			bool last = false;
			if (t + 1.01 * h > t1)
			{
				h = t1 - t;
				last = true;
			}
			step += 1;

			double k1 = f(t, y);
			double k2 = f(t + c2 * h, y + (c2 * h) * (a21 * k1));
			double k3 = f(t + c3 * h, y + (c3 * h) * (a31 * k1 + a32 * k2));
			double k4 = f(t + c4 * h, y + (c4 * h) * (a41 * k1 + a42 * k2 + a43 * k3));
			double k5 = f(t + c5 * h, y + (c5 * h) * (a51 * k1 + a52 * k2 + a53 * k3 + a54 * k4));
			double y6 = y + h * (a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5);
			double k6 = f(t + h, y6);
			double y7 = y + h * (a71 * k1 + a73 * k3 + a74 * k4 + a75 * k5 + a76 * k6);
			double k7 = f(t + h, y7);

			double err = std::abs((h * (e1 * k1 + e3 * k3 + e4 * k4 + e5 * k5 + e6 * k6 + e7 * k7)) / (tolabs + tolrel * std::max<double>(std::abs(y), std::abs(y7))));
			double hnew = h / std::max<double>(facc2, std::min<double>(facc1, std::pow(err, expo1) / std::pow(facold, beta) / safe));

			if (err < 1.0)
			{
				// Current step is accepted.
				naccept += 1;
				facold = std::max<double>(err, 1e-4);
				// Stiffness detection
				if ((naccept % nstiff == 0) || (iasti > 0))
				{
					double num = std::pow(k7 - k6, 2.0);
					double den = std::pow(y7 - y6, 2.0);
					if (den > 0.0)
					{
						hlamb = h * std::sqrt(num / den);
					}
					if (hlamb > 3.25)
					{
						nonsti = 0;
						iasti += 1;
						if (iasti == 15)
						{
							return false;
						}
					}
					else
					{
						nonsti += 1;
						if (nonsti == 6)
						{
							iasti = 0;
						}
					}
				}
				t += h;
				y = y7;
				if (last)
				{
					h = hnew;
					return true;
				}
				hnew = std::min<double>(hnew, hmax);
				if (reject)
				{
					hnew = std::min<double>(hnew, h);
					reject = false;
				}
			}
			else
			{
				// Current step is rejected.
				hnew = h / std::min<double>(facc1, std::pow(err, expo1) / safe);
				reject = true;
				if (naccept >= 1)
				{
					nreject += 1;
				}
			}
			h = hnew;
		}
		return false;
	}

	bool dopri5(
		const std::function<Eigen::VectorXd(double, const Eigen::VectorXd&)> f,
		const double t0,
		const double t1,
		const Eigen::Ref<const Eigen::VectorXd>& y0,
		const double tolrel,
		const double tolabs,
		Eigen::Ref<Eigen::VectorXd, Eigen::Unaligned, Eigen::InnerStride<Eigen::Dynamic>> y)
	{
		if (t1 == t0)
		{
			y = y0;
			return true;
		}

		constexpr double c2 = 0.2, c3 = 0.3, c4 = 0.8, c5 = 8.0 / 9.0;
		constexpr double a21 = 1.0;
		constexpr double a31 = 0.25, a32 = 0.75;
		constexpr double a41 = 11.0 / 9.0, a42 = -14.0 / 3.0, a43 = 40.0 / 9.0;
		constexpr double a51 = 4843.0 / 1458.0, a52 = -3170.0 / 243.0, a53 = 8056.0 / 729.0, a54 = -53.0 / 162.0;
		constexpr double a61 = 9017.0 / 3168.0, a62 = -355.0 / 33.0, a63 = 46732.0 / 5247.0, a64 = 49.0 / 176.0, a65 = -5103.0 / 18656.0;
		constexpr double a71 = 35.0 / 384.0, a73 = 500.0 / 1113.0, a74 = 125.0 / 192.0, a75 = -2187.0 / 6784.0, a76 = 11.0 / 84.0;
		constexpr double e1 = 71.0 / 57600.0, e3 = -71.0 / 16695.0, e4 = 71.0 / 1920.0, e5 = -17253.0 / 339200.0, e6 = 22.0 / 525.0, e7 = -1.0 / 40.0;

		// Maximal number of allowed steps
		constexpr int maxstep = 100000;
		// Step number after which stiffness detection is activated
		constexpr int nstiff = 1000;
		// Machine epsilon for double precision
		constexpr double eps = 2.3e-16;
		constexpr double safe = 0.9;
		constexpr double fac1 = 0.2;
		constexpr double fac2 = 10.0;
		constexpr double beta = 0.04;
		constexpr double expo1 = 0.2 - beta * 0.75;
		constexpr double facc1 = 1.0 / fac1;
		constexpr double facc2 = 1.0 / fac2;
		const double hmax = t1 - t0;

		// Stiffness detection variables
		int iasti = 0;
		int nonsti = 0;
		double hlamb = 0.0;

		bool reject = false;
		double facold = 1e-4;

		Eigen::VectorXd yc = y0;
		double h = 0.0;
		double t = t0;
		// Compute a first guess for explicit Euler as
		//     h = 0.01 * norm(y0) / norm(f(t0, y0))
		Eigen::VectorXd f0 = f(t0, yc);
		Eigen::VectorXd sk = tolabs + tolrel * yc.array().abs();
		double dnf = f0.cwiseQuotient(sk).squaredNorm();
		double dny = yc.cwiseQuotient(sk).squaredNorm();
		if ((dnf < 1e-10) || (dny < 1e-10))
		{
			h = 1e-6;
		}
		else
		{
			h = 0.01 * std::sqrt(dny / dnf);
		}
		// TODO: allow user-specified maximal step size
		h = std::min<double>(h, t1 - t0);
		Eigen::VectorXd f1 = f(t0 + h, yc + h * f0);
		double der2 = (f1 - f0).cwiseQuotient(sk).norm() / h;
		// Step size is computed such that
		//     h**5 * max(norm(f(t0, y0)), norm(der2)) = 0.01
		double der12 = std::max<double>(der2, std::sqrt(dnf));
		if (der12 < 1e-15)
		{
			h = std::min<double>({ 100.0 * h, std::max<double>(1e-6, h * 1e-3), t1 - t0 });
		}
		else
		{
			h = std::min<double>({ 100.0 * h, std::pow(0.01 / der12, 0.2), t1 - t0 });
		}

		int step = 0;
		int naccept = 0;
		int nreject = 0;
		while (true)
		{
			if (step > maxstep)
			{
				return false;
			}
			if (0.01 * h < std::abs(t) * eps)
			{
				return false;
			}
			bool last = false;
			if (t + 1.01 * h > t1)
			{
				h = t1 - t;
				last = true;
			}
			step += 1;

			Eigen::VectorXd k1 = f(t, yc);
			Eigen::VectorXd k2 = f(t + c2 * h, yc + (c2 * h) * (a21 * k1));
			Eigen::VectorXd k3 = f(t + c3 * h, yc + (c3 * h) * (a31 * k1 + a32 * k2));
			Eigen::VectorXd k4 = f(t + c4 * h, yc + (c4 * h) * (a41 * k1 + a42 * k2 + a43 * k3));
			Eigen::VectorXd k5 = f(t + c5 * h, yc + (c5 * h) * (a51 * k1 + a52 * k2 + a53 * k3 + a54 * k4));
			Eigen::VectorXd y6 = yc + h * (a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5);
			Eigen::VectorXd k6 = f(t + h, y6);
			Eigen::VectorXd y7 = yc + h * (a71 * k1 + a73 * k3 + a74 * k4 + a75 * k5 + a76 * k6);
			Eigen::VectorXd k7 = f(t + h, y7);

			Eigen::VectorXd sk = tolabs + tolrel * yc.array().abs().max(y7.array().abs());
			double err = (h * (e1 * k1 + e3 * k3 + e4 * k4 + e5 * k5 + e6 * k6 + e7 * k7)).cwiseQuotient(sk).norm() * std::sqrt(1.0 / yc.size());
			double hnew = h / std::max<double>(facc2, std::min<double>(facc1, std::pow(err, expo1) / std::pow(facold, beta) / safe));

			if (err < 1.0)
			{
				// Current step is accepted.
				naccept += 1;
				facold = std::max<double>(err, 1e-4);
				// Stiffness detection
				if ((naccept % nstiff == 0) || (iasti > 0))
				{
					double num = (k7 - k6).squaredNorm();
					double den = (y7 - y6).squaredNorm();
					if (den > 0.0)
					{
						hlamb = h * std::sqrt(num / den);
					}
					if (hlamb > 3.25)
					{
						nonsti = 0;
						iasti += 1;
						if (iasti == 15)
						{
							return false;
						}
					}
					else
					{
						nonsti += 1;
						if (nonsti == 6)
						{
							iasti = 0;
						}
					}
				}
				t += h;
				yc = y7;
				if (last)
				{
					y = yc;
					h = hnew;
					return true;
				}
				hnew = std::min<double>(hnew, hmax);
				if (reject)
				{
					hnew = std::min<double>(hnew, h);
					reject = false;
				}
			}
			else
			{
				// Current step is rejected.
				hnew = h / std::min<double>(facc1, std::pow(err, expo1) / safe);
				reject = true;
				if (naccept >= 1)
				{
					nreject += 1;
				}
			}
			h = hnew;
		}
		return false;
	}
}
