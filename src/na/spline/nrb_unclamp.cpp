	...
	namespace nurbs
	{
			...
			const Eigen::Index k)
		{
			...
			for (Eigen::Index ii = deg - kk - 1; ii <= deg - 2; ++ii)
			{
				for (Eigen::Index jj = ii; jj >= 0; --jj)
				{
					double alpha = (knots.coeffRef(n + 1 - jj + ii) - knots.coeffRef(n - jj - 1)) / (knots.coeffRef(n) - knots.coeffRef(n - jj - 1));
					coefs.row(n - jj - 1) = alpha * coefs.row(n - jj - 1) + (1.0 - alpha) * coefs.row(n - jj - 2);
				}
			}
